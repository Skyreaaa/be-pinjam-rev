...
const uploadFineProof = multer({ storage: multer.memoryStorage(), limits:{ fileSize: 5*1024*1024 }});
...
router.post('/upload-fine-proof', uploadFineProof.single('proof'), async (req,res)=>{
    ...
    try {
        ...
        let proofUrl = null;
        if (isConfigured()) {
            const result = await uploadBufferToCloudinary(req.file.buffer, { folder: 'fine-proofs', mimetype: req.file.mimetype });
            proofUrl = result.secure_url;
        } else {
            // Fallback lokal
            const p = require('path');
            const fsp = require('fs/promises');
            const dir = p.join(__dirname, '..', 'uploads', 'fine-proofs');
            await fsp.mkdir(dir, { recursive:true });
            const ext = p.extname(req.file.originalname || '').toLowerCase() || '.jpg';
            const fname = `fine-${Date.now()}-${Math.round(Math.random()*1e9)}${ext}`;
            await fsp.writeFile(p.join(dir, fname), req.file.buffer);
            proofUrl = '/uploads/fine-proofs/' + fname;
        }
